...
// ✅ 正确 - 只调用确认存在的方法
this.event.emit([Module]EventName.[EventKey], { [字段]: data.[字段] });  // ✅ this.event.emit 是 CCBusiness/CCView 的事件触发方法
...
| | 5 | Interface 文件（接口定义） |
| | 6 | Config 文件（配置定义） |
| **第2层：业务层 (Business)** | 7 | Event 文件（事件定义与事件数据） |
| | 8 | Business 文件（业务逻辑实现） |
| | 9 | System 文件（系统逻辑实现） |
| **第3层：显示层 (View)** | 10 | View 文件（界面视图实现） |
| | 11 | ViewPrefab 文件（预制体视图实现） |
...
| `this.event.emit(EventName, data)` | 触发事件 | `this.event.emit([Module]EventName.[EventKey], { [字段], [字段] });` |
...
- [ ] 检查 `declare global` 是否存在（Event 文件）
...
// ❌ 错误 - export type（已废弃，事件数据合并到 Event 文件中）

// ✅ 正确 - 事件枚举与事件数据在同一文件 [Module]Event.ts 中
```
...
| Business → View | 模块内事件 | `this.event.emit([Module]EventName.[EventKey])` |
| Business → Model | 直接操作 | `this.ent.M_[Module]_Main.[属性].set([键], [值])` |
| 跨模块 | 全局事件 | `this.event.emit([全局事件名].[EventKey], [数据])` |
