...
class InternalNodePool {
	...
	constructor(prefabs: Prefab[], nodes?: Node[]) {
	    ...
	    const count = this.useNodeMode ? this.nodes.length : prefabs.length;
...
export class VirtualScrollView extends Component {
	...
	triggerItemEnterCallbackOnce: boolean = false;
	...
	blockParentScroll: boolean = true;

	@property({
	    displayName: '限制起手角度',
	    tooltip: '开启后，仅当首次滑动角度与列表方向匹配时才开始滚动',
	})
	limitStartDragAngle: boolean = false;
	...
	startDragAngleThreshold: number = 30;
	...
	scrollStyle: ScrollStyle = ScrollStyle.APPLE;
	...
	enableMouseWheel: boolean = false;
	...
	mouseWheelSpeed: number = 3.0;

	// 渲染子项内容
	renderItemFn: RenderItemFn | null = null;
	// 动态提供子项节点
	provideNodeFn: ProvideNodeFn | null = null;
	// 子项点击事件
	onItemClickFn: OnItemClickFn | null = null;
	// 子项长按事件
	onItemLongPressFn: OnItemLongPressFn | null = null;
	// 子项显示前初始化事件
	onItemInitFn: OnItemInitFn | null = null;
	// 新增项首次布局动画
	playItemAppearAnimationFn: PlayItemAppearAnimationFn | null = null;
	// 子项边缘进入可视区事件
	onItemEdgeEnterFn: OnItemEdgeEnterFn | null = null;
	// 子项完全进入可视区事件
	onItemFullEnterFn: OnItemFullEnterFn | null = null;

	// 动态尺寸查询
	getItemHeightFn: GetItemHeightFn | null = null;
	// 动态模板类型查询
	getItemTypeIndexFn: GetItemTypeIndexFn | null = null;
	// 刷新状态变化事件
	onRefreshStateChangeFn: OnRefreshStateChangeFn | null = null;
	// 加载更多状态变化事件
	onLoadMoreStateChangeFn: OnLoadMoreStateChangeFn | null = null;
	// 分页切换事件
	onPageChangeFn: OnPageChangeFn | null = null;
	...
	private async _initFixedSizeMode() {
	    if (!this.provideNodeFn) {
	        this.provideNodeFn = (_index: number) => {
	            // Node 模式
	...
	updateItemHeight(index: number, newSize?: number) {
	    if (!this.useDynamicSize) {
	...
	updateItemHeights(updates: Array<{ index: number; height: number }>) {
	    if (!this.useDynamicSize) {
	...
	setScrollStyle(style: ScrollStyle) {
	...
	scrollToPosition(targetPos: number, animate = false, duration?: number, onComplete?: () => void) {
	...
	scrollToTop(animate = false, duration?: number, onComplete?: () => void) {
	...
	scrollToBottom(animate = false, duration?: number, onComplete?: () => void) {
	...
	scrollToIndex(index: number, animate = false, duration?: number, onComplete?: () => void, center = false) {
	 ...
	 */
	finishRefresh(success: boolean = true) {
	 ...
	 */
	finishLoadMore(hasMore: boolean = true) {
	...
	private async _layoutSingleSlot(node: Node | null, idx: number, slot: number) {
	    if (!this.useVirtualList) return;
	    if (this.useDynamicSize) {
	        if (!this.getItemTypeIndexFn) return;
	...
	private _playDefaultItemAppearAnimation(_node: Node, _index: number) { }
	...
	private _layoutSlots(firstIndex: number, _forceRender: boolean) {
	    if (!this.useVirtualList) return;
	 ...
	 */
	getItemByIndex(index: number): Node | null {
	 ...
	 * 滚动到指定页
	 */
	scrollToPage(pageIndex: number, animate: boolean = true, onComplete?: () => void) {
	    if (!this.enablePageSnap) {
