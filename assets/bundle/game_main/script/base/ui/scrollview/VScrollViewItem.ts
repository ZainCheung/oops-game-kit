import { _decorator, Component, Node, EventTouch, Vec2, Label, settings, Sorting2D } from 'cc';
...
export class VScrollViewItem extends Component {
    ...
    useItemClickEffect: boolean = true;

    /** 点击回调（由 VirtualScrollView 注入） */
    onClickCallback: ((index: number) => void) | null = null;

    /** 长按回调（由 VirtualScrollView 注入） */
    onLongPressCallback: ((index: number) => void) | null = null;

    /** 长按触发时长（秒） */
    longPressTime: number = 0.6;
