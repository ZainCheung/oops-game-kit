...
import { ScreenshotHelper } from '../../ui/ScreenshotHelper';
...
export class B_Share_Main extends CCBusiness<Share> {
    ...
    /**
     * 截图分享 - 跨平台统一流程：
     *  1. screenshotData 为空 → 调 ScreenshotHelper.capture() 截图
    ...
    private async onShareScreenshot<K extends ShareEventName.ShareScreenshot>(
        ...
        try {
            // 1. 没有现成截图 → 走 ScreenshotHelper 截图
            if (!data.screenshotData) {
                data.screenshotData = await ScreenshotHelper.capture(this.sdk.readFileAsBase64, {
                    scale: WeChatSdkCfg.capture.scale,
